...
This project explores machine learning.
...
Use the random forest on the training dataset.
Random forest algorithm includes cross-validation. Then use the trained model to predict values with test dataset.Set seed for reproducibility.
```{r random_forest_train}
set.seed(32343)
rf <- randomForest(classe ~ ., data = df2)
rf
```
Type of random forest is classification. 500 trees.Out-of-box estimate of error rate is 1.03%
